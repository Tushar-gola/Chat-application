...
import StyleBadge from '../components/mui/StyleBadge'
            ...
            <Grid item xs={12}>
              <ul style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {currentUser.length > 0 ? currentUser && currentUser.map((user) => (
                  <li key={user.id} onClick={() => { handleItemClick(user.id); dispatch(open()) }} className={`user-details py-1 px-4 ${activeItem === user.id ? 'active' : ''}`}>
